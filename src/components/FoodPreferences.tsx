...
export default function FoodPreferences({
  ...
  const likeCategories = preferenceCategories.filter((category) =>
    ["cuisines", "diets", "likes"].includes(category.key),
  );
  const dislikeCategories = preferenceCategories.filter((category) =>
    ["allergies", "dislikes"].includes(category.key),
  );
      ...
      <CardContent className={compact ? "p-4 pt-2" : undefined}>
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {preferences[category.key as StringArrayKey].map((item) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  handleRemovePreference(category.key as StringArrayKey, item)
                }
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-md border px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:outline-hidden"
          />
          <Button
            size="sm"
            onClick={() => {
